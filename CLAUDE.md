...
8. **제스처 캔버스 위에 올라가는 모든 표시 요소(오버레이·패널·토스트·시범 궤적/연출·페이즈 안내·상태 표시)는 기본 `pointer-events: none`** — 보이기만 하고 터치는 아래 캔버스로 통과시킨다. 버튼 등 실제 인터랙션 요소만 예외적으로 `pointer-events: auto`. **전 씬(자유/수련/결전/승급)에서 화면 전체가 검로 제스처 영역**이어야 한다(중앙 표시 상자가 터치를 삼키면 안 됨 — 버그4 재발 방지).
