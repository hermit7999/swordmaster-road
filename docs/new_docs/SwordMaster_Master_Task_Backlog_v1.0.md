...
✅(2차) 트윈스틱 UI(가이드 링·이동/검격 라벨·터치 회피 «)·좌수검 미러 유파·햅틱 진동·잉크 스플래시
✅(3차) UI-07 프롤로그 오프닝 — 프로토타입 대본 이식(멸문의 밤→스승 현곡→첫 획→유파 선택[좌수/우수]→스테이지1). DialogueScene(초상화+타이핑+탭+선택+첫 획 관대 판정), 2회차부터 스킵. 흐름: 타이틀→대화→게임.
✅ 배포 파이프라인 — GitHub Pages에 game/ 병행 배포(hermit7999.github.io/swordmaster-road/game/), push.bat 원클릭
잔여 ↓
