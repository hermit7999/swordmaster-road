...
NEG = ("color photo, colorful background, cluttered background, multiple characters, "
       "text, watermark, signature, blurry, low quality, jpeg artifacts, deformed, "
       "bad anatomy, extra limbs, extra arrows, cropped, frame, border")
...
def main():
    ...
    ap.add_argument("--guidance", type=float, default=1.8)
    ...
    try:
        ...
        print(f"[gen] 앵커: {anchor_path.name}  프롬프트: {args.prompt[:60]}...")
        ...
        init = Image.new("RGB", (W, H), bg_color)      # 전체 마스크라 내용은 무의미
