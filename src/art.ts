 ...
 *  성공 시에만 이미지+먹빛 그라데이션 오버레이를 얹고, 실패하면 배경을 건드리지 않는다. */
let bgToken = 0;
export function setSceneBg(name: string | null, combat = false): void {
  const el = document.querySelector('#sceneBg') as HTMLElement | null;
  if (!el) return;
  const token = ++bgToken;
  if (!name) { el.style.backgroundImage = ''; el.classList.remove('on'); return; }
  loadArt(name).then((img) => {
    if (token !== bgToken) return;      // 더 최근 씬 전환이 있었으면 무시
    if (!img) { el.style.backgroundImage = ''; el.classList.remove('on'); return; } // 폴백: 배경 없음
    const a = combat ? BALANCE.art.bgOverlayCombat : BALANCE.art.bgOverlay;
    el.style.backgroundImage =
      `linear-gradient(rgba(18,15,12,${a}), rgba(18,15,12,${a})), url("${artUrl(name)}")`;
    el.classList.add('on');
  });
}
